<?php
        if ($type == 'Planetes') {
            ...
            echo "<p> <a href=\"?action=delete&controller=administrateur&type=Planetes&id=" .
             htmlspecialchars($o->get('id')) . "\"> Supprimer </a>   <a href=\"?action=gotoupdate&controller=administrateur&type=Planetes&id=" . $id . "\"> Modifier </a> </p>";
        } elseif ($type == 'Client') {
            $id = $o->get('login');
            $id = rawurlencode($id);
